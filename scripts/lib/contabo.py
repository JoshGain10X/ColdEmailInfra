...
class ContaboClient:
    ...
    def ensure_ssh_user(self, instance_id: int, ssh_key_id: int, username: str = "admin") -> dict:
        """No-op on Contabo: the sshKeys array on POST /compute/instances
        attaches the key to the default `admin` user at provision time,
        which already has passwordless sudo. No bootstrap password is
        required — returns `password=None` so the caller skips the sudo
        bootstrap step.
        """
